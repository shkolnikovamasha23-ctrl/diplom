...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password_hash'], password):
            ...
            get_db().execute('INSERT INTO activity_logs(user_id, action, details, created_at) VALUES (?,?,?,?)', (user['id'], 'login', 'Пользователь вошел в систему', datetime.now().isoformat(timespec='seconds')))
            get_db().commit()
...
def register():
    if request.method == 'POST':
        ...
        if errors:
            ...
            try:
                db.execute('INSERT INTO users(username,password_hash,full_name,email,role_id,created_at,is_active) VALUES (?,?,?,?,?,?,1)', (clean_text(request.form['username'], 80), generate_password_hash(request.form['password']), clean_text(request.form['full_name'], 160), clean_text(request.form['email'], 160), role['id'], datetime.now().isoformat(timespec='seconds')))
                db.commit()
