"""Административная панель сетевого ресурса."""
...
def dashboard():
    ...
    stats = {
        ...
        "feedback": db.execute("SELECT COUNT(*) FROM feedback WHERE status='new'").fetchone()[0],
    }
...
def scenarios():
    ...
    if request.method == "POST":
        db.execute("INSERT INTO scenarios(title,slug,difficulty,duration_minutes,summary,learning_goal,is_public) VALUES (?,?,?,?,?,?,1)",
                   (clean_text(request.form.get("title"), 180), clean_text(request.form.get("slug"), 120), clean_text(request.form.get("difficulty"), 60), int(request.form.get("duration_minutes", 20)), clean_text(request.form.get("summary"), 500), clean_text(request.form.get("learning_goal"), 500)))
        db.commit()
