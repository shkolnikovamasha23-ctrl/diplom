"""Публичные страницы сайта, доступные без авторизации."""
...
PUBLIC_PAGES = {
    "about": {"title": "О проекте", "template": "public/about.html"},
    "methodology": {"title": "Методика", "template": "public/methodology.html"},
    "cases": {"title": "Учебные кейсы", "template": "public/cases.html"},
    "faq": {"title": "FAQ", "template": "public/faq.html"},
    "contacts": {"title": "Контакты", "template": "public/contacts.html"},
    "help": {"title": "Справка", "template": "public/help.html"},
    "accessibility": {"title": "Доступность", "template": "public/accessibility.html"},
    "privacy": {"title": "Данные", "template": "public/privacy.html"},
}
...
def feedback():
    if request.method == "POST":
        ...
        if errors:
            ...
            get_db().execute("INSERT INTO feedback(name,email,topic,message,status,created_at) VALUES (?,?,?,?,?,?)",
                             (clean_text(request.form["name"], 120), clean_text(request.form["email"], 160), clean_text(request.form["topic"], 160), clean_text(request.form["message"], 2000), "new", datetime.now().isoformat(timespec="seconds")))
            get_db().commit()
